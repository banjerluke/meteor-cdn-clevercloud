...
function CdnController(){
	...
	function setClientCdnUrl(cdnUrl){
		...
		WebAppInternals.setBundledJsCssUrlRewriteHook(function(url) {
			// Fixes a potential issue where you get served stale code after deployments.
			// Based on Gaxaxy-specific code, but adapted for use with Clever Cloud.
			var instanceId = process.env.INSTANCE_ID;
			var rewrittenUrl = cdnUrl + url;
			if (instanceId) {
				var separator = hasQuestionMark.test(url) ? '&' : '?';
				rewrittenUrl += separator + '_inst_id_=' + instanceId;
			}
			return rewrittenUrl;
		});
